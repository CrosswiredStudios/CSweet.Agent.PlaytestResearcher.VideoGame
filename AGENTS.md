# Video Game Playtest Researcher repository instructions

This repository contains one standalone C-Sweet protocol-v2 agent. Its purpose is:

> Plans consent-governed evaluations and produces evidence-backed playtest findings.

## Invariants

- Keep `com.csweet.video-game-playtest-researcher` and version `1.0.0` synchronized between agent code,
...
```powershell
dotnet test
dotnet run --project src/CSweet.Agent.PlaytestResearcher.VideoGame -- --self-test
```
